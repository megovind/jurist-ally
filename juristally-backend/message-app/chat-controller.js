...
exports.add_user_list = async (from, to) => {
    try {
        await add_user(from, to);
        await add_user(to, from);
        return;
    } catch (error) {
        return { status: 'ERROR' };
    }
...
exports.fetch_chatlist = async (userId) => {
    try {
        ...
        if (_.isEmpty(response)) return json({ status: "ERROR", message: 'Chat list not found' });
        return response;
    } catch (error) {
...
exports.fetch_all_message = async (from, to) => {
    try {
        ...
        if (_.isEmpty(response)) return json({ status: "ERROR", message: 'Not found!' });
        return response;
    } catch (error) {
        return json({ status: 'ERROR', message: error.message });
    }
